...
export default function FloatingWhatsApp({ content }: { content: SiteContent }) {
  ...
  return (
    <Link
      ...
      target="_blank"
      rel="noreferrer"
      className="fixed bottom-[calc(env(safe-area-inset-bottom)+3.5rem)] right-3 z-50 inline-flex h-10 w-10 items-center justify-center gap-2 rounded-full bg-emerald-600 text-sm font-bold text-white shadow-2xl shadow-slate-900/20 hover:bg-emerald-700 sm:bottom-20 sm:right-5 sm:h-12 sm:w-auto sm:px-5"
    >
