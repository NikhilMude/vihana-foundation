...
export function Logo({ brandName = "Vihana", brandTagline = "Foundation", logoImageUrl = "" }: LogoProps) {
  return (
    <SmartNavLink href="/#home" className="flex min-w-0 items-center gap-2 sm:gap-3" aria-label={`${brandName} ${brandTagline} home`}>
      {logoImageUrl ? (
        <Image
          src={logoImageUrl}
        ...
        <div className="flex h-9 w-9 items-center justify-center rounded-[8px] bg-teal-700 text-white shadow-sm sm:h-11 sm:w-11">
      ...
      <div className="min-w-0">
        ...
        <p className="truncate text-[8px] font-bold uppercase tracking-[0.22em] text-slate-500 sm:text-[10px] sm:tracking-[0.28em]">
          {brandTagline}
        </p>
      </div>
    </SmartNavLink>
