  ...
  return (
    <section id="donate" className="bg-stone-50 py-6 md:py-12">
      <Container>
        ...
        <Reveal>
          <div className="mt-4 grid overflow-hidden rounded-[8px] bg-slate-950 shadow-xl shadow-slate-900/10 lg:mt-8 lg:grid-cols-[1.05fr_0.95fr]">
            <div className="relative min-h-[170px] sm:min-h-[260px] lg:order-2">
              <Image
        ...
        <div className="mt-4 grid gap-3 lg:mt-8 lg:grid-cols-3 lg:gap-5">
          {donationOptions.map((option, index) => {
            const Icon = option.icon;

            return (
              <Reveal key={option.title} delay={index * 0.06}>
                <div className="h-full rounded-[8px] border border-slate-200 bg-white p-4 shadow-sm sm:p-6">
                  <div className="flex h-9 w-9 items-center justify-center rounded-[8px] bg-teal-50 text-teal-700 sm:h-12 sm:w-12">
                    <Icon className="h-5 w-5 sm:h-6 sm:w-6" />
                  </div>

                  <h3 className="mt-3 text-base font-bold text-slate-950 sm:mt-6 sm:text-xl">{option.title}</h3>

                  <div className="mt-2 flex items-center justify-between gap-3 rounded-[8px] bg-slate-50 px-3 py-2 sm:px-4 sm:py-3">
                  ...
                  <p className="mt-2 text-sm leading-5 text-slate-500 sm:mt-4 sm:leading-6">{option.note}</p>
                </div>
              </Reveal>
            );
          })}
        ...
        <Reveal>
          <div className="mt-4 rounded-[8px] border border-teal-200 bg-white p-4 shadow-sm sm:mt-8 sm:p-6">
