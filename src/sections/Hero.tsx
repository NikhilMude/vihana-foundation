...
export default function Hero({ content }: { content: SiteContent }) {
  return (
    <section id="home" className="relative overflow-hidden bg-[linear-gradient(135deg,#f8fafc_0%,#ecfeff_48%,#fff7ed_100%)] pt-14 sm:pt-24">
      <div className="absolute inset-x-0 top-0 h-28 bg-[linear-gradient(180deg,rgba(255,255,255,0.92),rgba(255,255,255,0))] sm:h-40" />
      <Container className="grid items-center gap-4 pb-4 pt-4 sm:min-h-[calc(80vh-5rem)] sm:gap-9 sm:pb-10 sm:pt-7 lg:grid-cols-[1.02fr_0.98fr] lg:pb-12">
        <div className="relative z-10 max-w-3xl text-center lg:text-left">
          ...
          <h1 className="mx-auto mt-3 max-w-4xl font-[family-name:var(--font-playfair)] text-[2.1rem] font-bold leading-[0.93] text-slate-950 sm:mt-7 sm:text-6xl lg:mx-0 lg:text-7xl">
            {content.heroTitle}
            <span className="block text-teal-700">{content.heroHighlight}</span>
          </h1>

          <p className="mx-auto mt-2 max-w-2xl text-[13px] leading-5 text-slate-600 sm:mt-6 sm:text-xl sm:leading-8 lg:mx-0">
            {content.heroDescription}
          </p>

          <div className="mt-4 grid min-w-0 grid-cols-2 gap-2 sm:mt-8 sm:flex sm:flex-row sm:justify-center sm:gap-3 lg:justify-start">
            <Button asChild size="lg" className="h-9 min-w-0 rounded-full bg-teal-700 px-3 text-xs hover:bg-teal-800 sm:h-12 sm:w-auto sm:px-7 sm:text-base">
              <SmartNavLink href={content.heroPrimaryHref}>
                <HeartHandshake className="mr-1.5 h-4 w-4 sm:mr-2 sm:h-5 sm:w-5" />
                <span className="min-w-0 truncate">{content.heroPrimaryLabel}</span>
              </SmartNavLink>
            </Button>

            <Button asChild variant="outline" size="lg" className="h-9 min-w-0 rounded-full border-slate-300 bg-white/80 px-3 text-xs hover:bg-white sm:h-12 sm:w-auto sm:px-7 sm:text-base">
              <SmartNavLink href={content.heroSecondaryHref}>
                <span className="min-w-0 truncate">{content.heroSecondaryLabel}</span>
                <ArrowRight className="ml-1.5 h-4 w-4 sm:ml-2 sm:h-5 sm:w-5" />
              </SmartNavLink>
            </Button>
          </div>

          <div className="mx-auto mt-3 grid max-w-md grid-cols-3 gap-1.5 text-left sm:mt-8 sm:max-w-none sm:grid-cols-3 lg:mx-0">
            {promises.map((promise) => (
              <div key={promise} className="flex min-h-8 min-w-0 items-center gap-1 rounded-full bg-white/75 px-2 py-1 text-[10px] font-semibold leading-3 text-slate-700 shadow-sm sm:bg-transparent sm:px-0 sm:text-sm sm:shadow-none">
                <CheckCircle2 className="h-3.5 w-3.5 shrink-0 text-teal-700 sm:h-5 sm:w-5" />
        ...
        <div className="relative z-10 mx-auto w-full max-w-[20rem] sm:max-w-[26rem] lg:max-w-none">
          <div className="relative overflow-hidden rounded-[8px] border border-white/80 bg-[#fffdf8] p-2 shadow-2xl shadow-teal-950/10">
            <Image
              ...
              className="relative aspect-[16/9] w-full rounded-[8px] object-cover saturate-[0.92] sm:aspect-[4/3]"
            />
